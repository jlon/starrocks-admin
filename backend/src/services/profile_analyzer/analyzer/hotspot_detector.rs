...
impl HotSpotDetector {
    ...
    /// Parse duration string to seconds, supporting combined formats like "1h30m5s", "1m30s", "1s30ms"
    fn parse_duration(duration_str: &str) -> Result<f64, ()> {
        ...
        while i < chars.len() {
            ...
            if c.is_ascii_digit() || c == '.' {
                ...
                // Match unit (check multi-char units first)
                if c == 'h' {
                    ...
                    found_unit = true;
                    i += 2;
                } else if c == 'μ' && i + 1 < chars.len() && chars[i + 1] == 's' {
                    total_seconds += value / 1_000_000.0;
                    found_unit = true;
                    i += 2;
