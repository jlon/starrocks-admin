...
impl DiagnosticRule for F002MemorySkew {
    fn id(&self) -> &str { "F002" }
    fn name(&self) -> &str { "实例内存分配不均" }
    
    fn applicable_to(&self, _node: &ExecutionTreeNode) -> bool { true }
    
    fn evaluate(&self, context: &RuleContext) -> Option<Diagnostic> {
        let max_mem = context.get_metric("__MAX_OF_OperatorPeakMemoryUsage")?;
        let min_mem = context.get_metric("__MIN_OF_OperatorPeakMemoryUsage").unwrap_or(0.0);
        if min_mem == 0.0 { return None; }
...
impl DiagnosticRule for F003PrepareTimeLong {
    fn id(&self) -> &str { "F003" }
    fn name(&self) -> &str { "Fragment 准备时间过长" }
...
pub fn get_rules() -> Vec<Box<dyn DiagnosticRule>> {
    vec![
        Box::new(F001ExecutionTimeSkew),
        Box::new(F002MemorySkew),
        Box::new(F003PrepareTimeLong),
    ]
}
