...
use crate::services::profile_analyzer::parser::core::{
    SectionParser, TopologyParser, FragmentParser, OperatorParser, 
    TreeBuilder, MetricsParser, ValueParser,
};
...
impl ProfileComposer {
    ...
    fn build_nodes_from_topology_and_fragments(
        ...
        let mut operators_by_plan_id: HashMap<i32, Vec<(&crate::services::profile_analyzer::models::Operator, String, String)>> = HashMap::new();
        ...
        for topo_node in &topology.nodes {
            let tree_node = if let Some(op_list) = operators_by_plan_id.get(&topo_node.id) {
                ...
                let (frag_id, pipe_id) = op_list.first()
                ...
                if !aggregated_op.unique_metrics.is_empty() {
                    ...
                    let operator_text = Self::build_operator_text(&pure_name, topo_node.id, &aggregated_op);
    ...
    fn add_sink_nodes(&self, nodes: &mut Vec<ExecutionTreeNode>, fragments: &[Fragment], topology: &TopologyGraph) {
        ...
        for fragment in fragments {
            for pipeline in &fragment.pipelines {
                for operator in &pipeline.operators {
                    ...
                    let operator_text = OperatorParser::extract_operator_block(text, &operator.name, Some(plan_id));
    ...
    fn aggregate_operators(
        ...
        let mut matching_operators: Vec<&crate::services::profile_analyzer::models::Operator> = Vec::new();
        ...
        // Handle OLAP_SCAN / CONNECTOR_SCAN mapping
        ...
        for metric_name in &count_metrics {
            ...
            if total_count > 0 {
                base_operator.common_metrics.insert(metric_name.to_string(), total_count.to_string());
            }
        }
    ...
    fn parse_time_to_ms(time_str: &str) -> Option<f64> {
        ...
        if time_str.ends_with("us") {
            return time_str.trim_end_matches("us").parse::<f64>().map(|us| us / 1000.0).ok();
        }
        if time_str.ends_with("ns") {
            return time_str.trim_end_matches("ns").parse::<f64>().map(|ns| ns / 1_000_000.0).ok();
        }
        if time_str.ends_with("s") && !time_str.ends_with("ms") && !time_str.ends_with("us") && !time_str.ends_with("ns") {
            return time_str.trim_end_matches("s").parse::<f64>().map(|s| s * 1000.0).ok();
        }
    ...
    fn compute_top_time_consuming_nodes(nodes: &[ExecutionTreeNode], limit: usize) -> Vec<TopNode> {
        let mut sorted_nodes: Vec<_> = nodes.iter()
            .filter(|n| n.time_percentage.is_some() && n.time_percentage.unwrap() > 0.0 && n.plan_node_id.is_some())
            .collect();
    ...
    fn analyze_profile_completeness(text: &str, summary: &mut ProfileSummary) {
        ...
        static FRAGMENT_REGEX: Lazy<Regex> = 
            Lazy::new(|| Regex::new(r"Fragment \d+:").unwrap());
        ...
        summary.total_instance_count = if total_fragments > 0 { Some(total_fragments as i32) } else { None };
        summary.missing_instance_count = if missing_fragments > 0 { Some(missing_fragments as i32) } else { None };
        ...
        if !is_complete && total_fragments > 0 {
            let missing_pct = (missing_fragments as f64 / total_fragments as f64 * 100.0).round() as i32;
...
mod tests {
    ...
    fn test_extract_operator_name() {
        assert_eq!(ProfileComposer::extract_operator_name("OLAP_SCAN (plan_node_id=0)"), "OLAP_SCAN");
