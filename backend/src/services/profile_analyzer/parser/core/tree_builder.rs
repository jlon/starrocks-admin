...
use crate::services::profile_analyzer::models::{
    ExecutionTree, ExecutionTreeNode, ProfileSummary, Fragment, TopologyGraph,
...
impl TreeBuilder {
    ...
    pub fn build_from_topology(
        ...
        let root_idx = if let Some(sink_name) = sink_node_name {
            let sink_idx = nodes.iter().position(|n| n.operator_name == sink_name)
                .or_else(|| nodes.iter().position(|n| n.operator_name.ends_with("_SINK")));
    ...
    fn calculate_depths_from_root(nodes: &mut [ExecutionTreeNode], root_idx: usize) -> ParseResult<()> {
        if nodes.is_empty() {
            return Ok(());
        }
        ...
        for fragment in fragments {
            for pipeline in &fragment.pipelines {
                for operator in &pipeline.operators {
                    ...
                    let time_str = operator.common_metrics
...
mod tests {
    ...
    fn test_find_sink_node() {
        let fragments = vec![
            Fragment {
                ...
                instance_ids: vec![],
                pipelines: vec![
                    crate::services::profile_analyzer::models::Pipeline {
                        id: "0".to_string(),
                        metrics: HashMap::new(),
                        operators: vec![
    ...
    fn test_get_sink_priority() {
        assert!(TreeBuilder::get_sink_priority("RESULT_SINK") < TreeBuilder::get_sink_priority("OLAP_TABLE_SINK"));
        assert!(TreeBuilder::get_sink_priority("OLAP_TABLE_SINK") < TreeBuilder::get_sink_priority("EXCHANGE_SINK"));
    }
}
