...
use crate::services::profile_analyzer::models::{OperatorSpecializedMetrics, ExchangeSinkMetrics};
        ...
        for line in text.lines() {
            ...
            if let Some(colon_pos) = rest.find(": ") {
                ...
                match key {
                    ...
                    "DestFragmentIds" => {
                        metrics.dest_fragment_ids = value.split(',')
                            .map(|s| s.trim().to_string())
                            .collect();
                    }
                    "DestBeAddresses" => {
                        metrics.dest_be_addresses = value.split(',')
                            .map(|s| s.trim().to_string())
                            .collect();
                    }
                    _ => {}
