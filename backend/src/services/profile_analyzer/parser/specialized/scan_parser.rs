...
static TABLE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"Table:\s*(\S+)").unwrap()
});

static ROLLUP_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"Rollup:\s*(\S+)").unwrap()
});
...
impl ScanMetricsParser {
    ...
    fn parse_common_scan_metrics(&self, text: &str) -> ScanMetrics {
        ...
        if let Some(cap) = TABLE_REGEX.captures(text) {
            metrics.table = cap.get(1).map(|m| m.as_str().to_string()).unwrap_or_default();
        }
        
        // Extract rollup
        if let Some(cap) = ROLLUP_REGEX.captures(text) {
            metrics.rollup = cap.get(1).map(|m| m.as_str().to_string()).unwrap_or_default();
        }
