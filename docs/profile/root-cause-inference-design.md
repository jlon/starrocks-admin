...
## 附录 G: v7.0 LLM 增强的智能根因分析

> **版本**: v7.0
> **日期**: 2024-12-08
> **状态**: 设计中
> **核心改进**: 引入 LLM 进行开放域推理，突破规则系统的局限性

### G.1 v6.0 的根本局限

| 局限 | 说明 | 示例 |
|------|------|------|
| **规则硬编码** | 所有因果关系需要手工定义 | 新场景需要开发新规则 |
| **建议通用化** | 无法根据具体 SQL 给出针对性建议 | "优化分桶键" vs "将 user_id 改为 order_date" |
| **缺乏业务理解** | 不理解业务语义 | 无法识别"订单表按日期分区更合理" |
| **无法学习** | 不能从历史案例中学习 | 相同问题重复分析 |
| **解释能力弱** | 只能输出模板化文本 | 无法回答用户追问 |

### G.2 LLM 服务抽象层设计

#### G.2.1 设计原则

**LLM 能力是一个通用接口**，根因分析只是它的一个实现场景。未来还有：

| 场景 | System Prompt | 输入 | 输出 |
|------|--------------|------|------|
| **根因分析** | 性能专家 | Profile + 规则诊断 | 根因 + 因果链 + 建议 |
| **SQL 优化建议** | SQL 专家 | SQL + Schema | 优化后的 SQL |
| **参数推荐** | 配置专家 | 查询特征 + 当前参数 | 参数调整建议 |
| **表结构建议** | DDL 专家 | 表 Schema + 查询模式 | 分桶/分区建议 |
| **异常诊断** | 运维专家 | 错误日志 + 状态 | 原因 + 解决方案 |

#### G.2.2 LLM 服务 Trait 设计
...
经过调研，推荐使用 **async-openai** 库：

| 库名 | Stars | 特点 | 推荐度 |
|------|-------|------|--------|
| **async-openai** | 1.5k+ | 功能全面、异步、流式支持、Azure 兼容 | ⭐⭐⭐⭐⭐ |
| openai-api-rs | 500+ | 简单易用，功能较少 | ⭐⭐⭐ |
| openai-dive | 300+ | 异步支持好 | ⭐⭐⭐ |

**async-openai 特点**:
- 基于 tokio + reqwest 的异步实现
- 支持 SSE 流式响应
- 支持 OpenAI / Azure / 兼容 API (如 DeepSeek, 通义千问)
- Builder 模式构建请求
- 自动重试和速率限制处理
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Q1: 如何支持多个 LLM 提供商？(OpenAI/DeepSeek/Azure...)              │   │
│  │ A: llm_providers - 存储各提供商配置，支持切换和 fallback             │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Q2: 如何追踪 LLM 调用状态？（成功/失败/耗时/token数）                 │   │
│  │ A: llm_analysis_sessions - 记录每次调用的生命周期                    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Q3: 如何支持问题排查和重放？                                          │   │
│  │ A: llm_analysis_requests - 保存完整请求，可重现问题                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Q4: 如何存储和展示 LLM 分析结果？                                     │   │
│  │ A: llm_analysis_results - 结构化存储根因/因果链/建议                 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Q5: 如何避免重复调用 LLM？（相同 SQL 相同问题）                        │   │
│  │ A: llm_cache - 基于请求 hash 缓存响应，节省成本                      │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Q6: 如何监控 LLM 用量和成本？                                         │   │
│  │ A: llm_usage_stats - 聚合统计，支持成本核算                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│           ▼                                                                 │
│  ┌─────────────────┐     ┌─────────────────┐                               │
│  │ 2. 查询激活的    │────▶│ llm_providers   │                               │
│  │    LLM Provider │     │ (is_active=1)   │                               │
│  └────────┬────────┘     └─────────────────┘                               │
│           │                                                                 │
│           ▼                                                                 │
│  ┌─────────────────┐     ┌─────────────────┐                               │
...
│           ▼                                                                 │
│  ┌─────────────────┐     ┌─────────────────┐                               │
│  │ 5. 保存请求数据  │────▶│ llm_analysis_   │                               │
│  │    (for debug)  │     │ requests        │                               │
│  └────────┬────────┘     └─────────────────┘                               │
│           │                                                                 │
│           ▼                                                                 │
│  ┌─────────────────┐                                                       │
│  │ 6. 调用 LLM API │  ← 异步执行，1-5s                                      │
│  │    (OpenAI等)   │                                                       │
│  └────────┬────────┘                                                       │
│           │                                                                 │
│           ├── 成功 ──┐                                                      │
...
│           │  └─────────────────────────────────────────┘                   │
│           │                                                                 │
│           └── 失败 ──▶ 更新 Session (failed), 返回规则引擎结果              │
│                                                                             │
│  定时任务 (每日)                                                             │
│       │                                                                     │
│       ▼                                                                     │
│  ┌─────────────────┐     ┌─────────────────┐                               │
│  │ 11. 聚合统计    │────▶│ llm_usage_stats │                               │
│  └─────────────────┘     └─────────────────┘                               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                             │
│  ┌────────────────────┐       ┌────────────────────┐                       │
│  │  llm_providers     │       │  llm_analysis_     │                       │
│  ├────────────────────┤       │     sessions       │                       │
│  │ id (PK)            │       ├────────────────────┤                       │
│  │ name               │◄──────│ id (PK)            │                       │
│  │ api_base           │       │ provider_id (FK)   │                       │
│  │ model_name         │       │ query_id           │                       │
│  │ api_key_encrypted  │       │ cluster_id         │                       │
│  │ is_active ⭐       │       │ status             │                       │
│  │ max_tokens         │       │ created_at         │                       │
│  │ temperature        │       │ completed_at       │                       │
│  │ enabled            │       │ input_tokens       │                       │
...
│                                         ▼                                   │
│  ┌────────────────────┐       ┌────────────────────┐                       │
│  │ llm_analysis_      │       │  llm_analysis_     │                       │
│  │   requests         │       │    results         │                       │
│  ├────────────────────┤       ├────────────────────┤                       │
│  │ id (PK)            │       │ id (PK)            │                       │
│  │ session_id (FK)    │◄──────│ session_id (FK)    │                       │
│  │ request_json       │       │ root_causes_json   │                       │
│  │ sql_hash           │       │ causal_chains_json │                       │
│  │ profile_hash       │       │ recommendations_   │                       │
│  │ created_at         │       │   json             │                       │
│  └────────────────────┘       │ summary            │                       │
│                               │ hidden_issues_json │                       │
│                               │ confidence_avg     │                       │
...
### G.10 规则引擎与 LLM 结果融合设计

#### G.10.1 设计原则

**核心思路**: 规则引擎是"骨架"，LLM 是"血肉"
...
│  │                                                                      │   │
│  │  输出:                                                               │   │
│  │  • DiagnosticResult (节点级别诊断)                                   │   │
│  │  • AggregatedDiagnostic (聚合诊断)                                   │   │
│  │  • suggestions (参数调整建议)                                        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
...
│  │                                                                      │   │
│  │  输出:                                                               │   │
│  │  • root_causes (根因列表，含隐式)                                    │   │
│  │  • causal_chains (因果链)                                            │   │
│  │  • recommendations (含 SQL 示例)                                     │   │
│  │  • summary (自然语言总结)                                            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
...
│  │                          结果融合器 (Merger)                         │   │
│  │                                                                      │   │
│  │  1. 建议去重: 规则建议 ∪ LLM 建议，去除重复                          │   │
│  │  2. 优先级排序: 根因问题优先，症状问题次之                            │   │
│  │  3. 置信度标注: 规则=100%, LLM=confidence                            │   │
│  │  4. 来源标记: 标记每条建议的来源 (rule/llm/both)                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### G.10.2 融合后的 API 响应结构
...
│  │  │                                                              │   │   │
│  │  │ 🔴 根因 1: 统计信息过期 (置信度: 85%) [LLM]                   │   │   │
│  │  │    └── 导致: J002 Join顺序不优, J005 Broadcast表过大         │   │   │
│  │  │                                                              │   │   │
│  │  │ 🟡 根因 2: 分桶键选择不当 (置信度: 100%) [Rule]               │   │   │
│  │  │    └── 导致: S001 数据倾斜                                   │   │   │
│  │  │                                                              │   │   │
│  │  │ 📊 因果图: [可视化展示]                                       │   │   │
│  │  └─────────────────────────────────────────────────────────────┘   │   │
...
│  │  │ 💡 优化建议 (按优先级排序)                                    │   │   │
│  │  │                                                              │   │   │
│  │  │ 1. [根因] ANALYZE TABLE orders; [LLM] ⭐                     │   │   │
│  │  │ 2. [根因] 修改分桶键为 order_date [Rule+LLM] ⭐              │   │   │
│  │  │ 3. [症状] 增加 JOIN 并行度 [Rule]                            │   │   │
│  │  │ 4. [症状] 启用 Runtime Filter [Rule]                         │   │   │
│  │  └─────────────────────────────────────────────────────────────┘   │   │
│  │                                                                      │   │
│  │  ⭐ = 解决根因的建议 (优先执行)                                     │   │
│  │                                                                      │   │
...
│  │                                                                      │   │
│  │  ┌─────────────────────────────────────────────────────────────┐   │   │
│  │  │ 📍 节点: HASH_JOIN (plan_node_id: 5)                         │   │   │
│  │  │                                                              │   │   │
│  │  │ 🔍 该节点的诊断问题:                                          │   │   │
│  │  │                                                              │   │   │
│  │  │ ⚠️ J002: Join 顺序可能不优                                   │   │   │
│  │  │    原因: 小表在左侧，大表在右侧                               │   │   │
│  │  │    证据: left_rows=1000, right_rows=10000000                 │   │   │
│  │  │                                                              │   │   │
│  │  │ ⚠️ J005: Broadcast 表过大                                    │   │   │
│  │  │    原因: Broadcast 数据量 > 100MB                            │   │   │
│  │  │    证据: broadcast_bytes=500MB                               │   │   │
│  │  │                                                              │   │   │
│  │  │ 💡 建议:                                                     │   │   │
│  │  │ • ANALYZE TABLE xxx; (更新统计信息)                          │   │   │
│  │  │ • SET broadcast_row_limit = 500000; (调整阈值)               │   │   │
│  │  │                                                              │   │   │
│  │  │ 🔗 关联根因: 统计信息过期 (查看总览)                          │   │   │
│  │  └─────────────────────────────────────────────────────────────┘   │   │
│  │                                                                      │   │
...
#### G.11.2 总览 vs 节点详情的区别

| 维度 | 总览页 (Overview) | 节点详情 (Node Detail) |
|------|------------------|----------------------|
| **触发条件** | 未选择节点 / 点击"总览"按钮 | 点击 DAG 树中的节点 |
| **根因分析** | ✅ 显示全局根因 (含 LLM) | ❌ 不显示 (链接到总览) |
| **因果图** | ✅ 显示完整因果图 | ❌ 不显示 |
| **诊断问题** | ✅ 聚合显示 (aggregated_diagnostics) | ✅ 显示该节点的诊断 (node_diagnostics[id]) |
| **建议** | ✅ 全局建议 (merged_recommendations) | ✅ 节点级别建议 (诊断的 suggestions) |
| **数据来源** | aggregated_diagnostics + llm_analysis | node_diagnostics[plan_node_id] |
...
export class ProfileDiagnosticComponent {
  ...
  get currentDiagnostics(): DiagnosticResult[] {
    if (this.selectedNodeId === null) {
      // 总览: 返回聚合诊断 (用于显示)
      return [];  // 使用 aggregated_diagnostics
    } else {
      // 节点详情: 返回该节点的诊断
    ...
    <nb-card>
      <nb-card-header>
        ...
        <nb-badge *ngIf="llmAnalysis?.status === 'pending'" 
                  status="warning" text="AI 分析中..."></nb-badge>
      </nb-card-header>
      ...
      <nb-card-body>
        ...
        <ng-container *ngIf="!selectedNodeId">
          ...
          <nb-card *ngIf="showRootCauseAnalysis" accent="primary">
            <nb-card-header>🔍 根因分析</nb-card-header>
            <nb-card-body>
              <app-root-cause-list 
          ...
          <!-- 诊断问题列表 (聚合) -->
          <nb-card>
            <nb-card-header>⚠️ 诊断问题</nb-card-header>
            <nb-card-body>
          ...
          <nb-card accent="success">
            <nb-card-header>💡 优化建议</nb-card-header>
            <nb-card-body>
...
| 维度 | v6.0 (规则驱动) | v7.0 (LLM增强) |
|------|----------------|----------------|
| **隐式根因识别** | 需要手写规则 | ✅ 开放域推理 |
| **SQL 优化建议** | 通用模板 | ✅ 针对性 SQL 示例 |
| **因果解释** | 模板化文本 | ✅ 自然语言解释 |
| **业务理解** | ❌ | ✅ 可理解业务语义 |
| **学习能力** | ❌ | ✅ 可从反馈中改进 |
| **延迟** | < 10ms | 1-5s (可缓存) |
...
| 阶段 | 任务 | 工作量 | 优先级 |
|------|------|--------|--------|
| **Phase 1** | SQLite 表结构实现 | 1 天 | P0 |
| **Phase 2** | LLM 服务抽象层 (Trait) | 0.5 天 | P0 |
| **Phase 3** | LLM 客户端封装 (async-openai) | 1 天 | P0 |
| **Phase 4** | 请求/响应数据结构 | 1 天 | P0 |
| **Phase 5** | 规则引擎 + LLM 结果融合 | 1.5 天 | P0 |
| **Phase 6** | 缓存和持久化 | 1 天 | P1 |
| **Phase 7** | API 端点实现 | 1 天 | P1 |
| **Phase 8** | 前端集成 (ngx-admin) | 2 天 | P1 |
| **Phase 9** | 测试和调优 | 1 天 | P1 |
| **总计** | | **11 天** |
...
变更记录:
- v7.0 (2024-12-08): 新增 LLM 增强设计
  - G.2: LLM 服务抽象层设计 (Trait, 支持多场景)
  - G.3: LLM 增强架构图
  - G.4: Rust LLM 库选型 (async-openai)
  - G.5: LLM API 请求/响应设计
  - G.6: System Prompt 设计
  - G.7: LLM 客户端实现
  - G.8: SQLite 数据表设计 (7 张表 + 流程说明)
    - 修改 is_default → is_active (支持单一激活)
  - G.9: 完整调用流程
  - G.10: 规则引擎与 LLM 结果融合设计
  - G.11: 前端展示设计 (总览 vs 节点详情)
  - G.12: v7.0 vs 之前版本对比
  - G.13: 实施计划 (11 天)
  - G.14: 配置示例
